...
public class AspectInstructionCodeGenerator extends CodeGenerator<Set<AspectSpec>> {
    ...
    private void process(TypeSpec.Builder instructionClassBuilder, CodeBlock.Builder staticCodeBlockBuilder, AspectSpec spec, int index) {
        staticCodeBlockBuilder.add("configs[$L]=$S;\n", index, spec.getConfigClassName());
        staticCodeBlockBuilder.add("processors[$L]=$S;\n", index, spec.getProcessorClassName());
//        instructionClassBuilder.addField(FieldSpec.builder(String.class, "CREPE_" + spec.getConfigClassName().toUpperCase().replaceAll("\\.", "_"), Modifier.PRIVATE, Modifier.FINAL, Modifier.STATIC)
//                .initializer("$S", spec.getConfigClassName())
//                .build());
